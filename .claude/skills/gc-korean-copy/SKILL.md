...
| Candidate review | Server candidates use "결과지에 이렇게 적혀 있나요?" with an adjacent example-data limitation and the source page; digest stays in disclosure. Mobile actions use "제외 / 수정 / 확인" with descriptive accessible names. Source unavailable means confirmation is unavailable. |
| Server states | Show a plain Korean sentence; preserve the actual server status word in labelled technical details; never invent progress the server has not reported. |
