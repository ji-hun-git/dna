...
The repository currently has a Next.js presentation application and a Spring Boot skeleton. TypeScript modules under `apps/web/lib` model OAuth, consent, model-runner, and research contracts, but there are no Next route handlers. Spring has three ports, no controllers, no persistence adapters, and no migrations. Allowing both runtimes to grow into public authorities would duplicate session, authorization, consent, and health-record rules.
...
- Same-origin ingress, session storage, PostgreSQL, object storage, and the ingestion worker are **NOT IMPLEMENTED**.

## Migration plan

1. Preserve the TypeScript contracts as adversarial fixtures; do not create Next API routes.
2. Implement a local identity broker substitute, opaque session, and central authorization service in Spring.
3. Add PostgreSQL/Flyway and a synthetic-only consent-to-record schema.
4. Add quarantine object storage and a network-isolated deterministic worker.
5. Route one synthetic lifecycle through Spring and persistence; replace React-memory success states with server state.
...
- Current Spring absence is recorded in `docs/status/2026-08-30/backend-report.md`.
