...
| Show two dated values of the same item side by side | User-confirmed values only; no interpretation, colour, arrow semantics, threshold, or advice | IMPLEMENTED WAVE 2 (synthetic only; CI evidence recorded on merge) | `apps/web/tests/compare-records.test.ts`, `record-comparison.test.tsx`, copy scan, `foundation-lifecycle.spec.ts` |
