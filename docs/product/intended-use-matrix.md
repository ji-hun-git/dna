...
| Prepare questions for a professional | Fixed, neutral, source-linked question set attached to user-confirmed records; no interpretation, threshold, or advice | VERIFIED SYNTHETIC ONLY (CI run 33576825427 on 33e6ac8 and main run 33592526743 on 07d77fe: web tests, PostgreSQL lifecycle, browser lifecycle) | `apps/web/tests/visit-preparation.test.tsx`, copy scan, `foundation-lifecycle.spec.ts` |
| Show two dated values of the same item side by side | User-confirmed values only; no interpretation, colour, arrow semantics, threshold, or advice | VERIFIED SYNTHETIC ONLY (CI run 33594016798 on 0836d30: web tests, PostgreSQL lifecycle, two-document browser lifecycle) | `apps/web/tests/compare-records.test.ts`, `record-comparison.test.tsx`, copy scan, `foundation-lifecycle.spec.ts` |
