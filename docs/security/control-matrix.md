...
| Hostile file gate | OWASP ASVS file handling | parser exploit/malware | size, magic, digest allowlist, generated path, no-shell ClamAV adapter | checksum-pinned ClamAV 1.5.4 clean/detection CI test plus rejected-digest and extraction-block tests | Security | PARTIAL | no hosted isolation or operational official-signature feed | 2026-08-30 |
...
| Runtime artifact integrity | SLSA/Sigstore/CycloneDX | image substitution, unsigned build, unknown components | immutable digest publication, keyless Cosign, SLSA provenance, CycloneDX attestation, exact workflow identity | publication run 33370021596 and evidence artifacts | Supply chain | PARTIAL / STOP-SHIP | signatures pass, but all three GHCR packages were anonymously pullable on 2026-09-02 despite private intent | 2026-09-02 |
