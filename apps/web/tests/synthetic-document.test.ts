...
it("puts the fixed catalogue values in inspectable synthetic source bytes", () => {
  ...
  for (const text of ["Cholesterol: 188 mg/dL 120-199", "HbA1c: 5.2 %", "Vitamin D: 42 ng/mL", "2026-07-28"]) expect(july).toContain(text);
  for (const text of ["Cholesterol: 194 mg/dL", "HbA1c: 5.4 %", "Vitamin D: 45 ng/mL", "Bilirubin: 0.8 mg/dL", "2026-01-15"]) expect(january).toContain(text);
  expect(buildSyntheticResultPdf("2026-07")).toEqual(buildSyntheticResultPdf("2026-07"));
  expect(january).not.toContain("120-199");
  expect(july).not.toContain("Bilirubin");
});
