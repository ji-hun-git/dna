...
export const providerDemoRecordSchema = z.object({
  ...
  providerName: z.string().startsWith("예시 "),
  providerType: z.enum(["종합병원", "병원", "의원", "건강검진센터"]),
  region: z.enum(["서울", "부산"]),
  address: z.string().endsWith("합성 주소"),
  telephone: z.null(),
...
export const priceDemoRecordSchema = z.object({
  ...
  itemName: z.string().endsWith("· 합성 항목"),
  currentAmountWon: z.number().int().positive(),
  effectivePeriod: z.literal("합성 시연 기간"),
  ...
  prices: [
    {
      ...
      itemName: "뇌 MRI · 합성 항목",
      currentAmountWon: 480000,
      effectivePeriod: "합성 시연 기간",
