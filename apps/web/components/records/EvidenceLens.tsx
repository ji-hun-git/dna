import { StatusLabel } from "@/components/evidence/StatusLabel";
import { formatKoreanDate, formatKoreanDateTime } from "@/lib/format/korean-date";

export type EvidenceLensRecord = {
  id: string;
  label: string;
  value: string;
  originalValue: string;
  unit: string;
  reference: string;
  sourceName: string;
  observedAt: string;
  sourceLocation: string;
  sourceDigest: string;
  extractedAt: string;
  confirmedAt: string;
  automation?: {
    layoutModel: string;
    semanticModel: string;
    evaluationGate: string;
    executionBoundary: string;
    artifactPolicy: string;
    disposition: string;
  };
};

export type EvidenceLensProps = {
  record: EvidenceLensRecord;
  onBack?: () => void;
};

function BackIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="m15 18-6-6 6-6" />
    </svg>
  );
}

function DocumentIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M7 3h7l4 4v14H7V3Z" />
      <path d="M14 3v5h5M10 12h5M10 16h5" />
    </svg>
  );
}

export function EvidenceLens({ record, onBack }: EvidenceLensProps) {
  const corrected = record.value !== record.originalValue;
  const marks = Array.from({ length: 40 }, (_, index) => index);

  return (
    <main className="gc-evidence-lens">
      <div className="gc-evidence-lens__shell">
        <header className="gc-evidence-lens__appbar">
          <button type="button" onClick={onBack} aria-label="이전 화면으로 돌아가기">
            <BackIcon />
          </button>
          <a href="#evidence" aria-label="앎 기록 출처">
            <span aria-hidden="true">앎</span>
            <strong>기록 출처</strong>
          </a>
          <span>예시 기록 · 실제 개인정보 없음</span>
        </header>

        <section className="gc-evidence-lens__intro" aria-labelledby="evidence-title">
          <p>기록 출처 · 확인 과정</p>
          <h1 id="evidence-title">이 값의 출처를<br aria-hidden="true" /> 확인해요</h1>
          <p>예시 원문 위치와 사용자가 확인하는 과정을 함께 보여드려요.</p>
        </section>

        <section className="gc-evidence-lens__stage" aria-label={`${record.label} 근거 기록`}>
          <article className="gc-evidence-lens__ledger">
            <header>
              <span>앎 · 건강 기록 예시</span>
              <span>{formatKoreanDate(record.observedAt)}</span>
            </header>

            <div className="gc-evidence-lens__ledger-heading">
              <div>
                <p>검사 항목</p>
                <h2>{record.label}</h2>
              </div>
              <StatusLabel status="verified" />
            </div>

            <p className="gc-evidence-lens__value">
              <strong>{record.value}</strong>
              <span>{record.unit}</span>
            </p>

            <div
              className="gc-evidence-lens__matrix"
              role="img"
              aria-label="원문, 자동 추출 후보, 사용자 확인, 저장의 네 단계가 모두 연결됨"
            >
              {marks.map((mark) => <span key={mark} data-active={mark < 32 ? "true" : "false"} aria-hidden="true" />)}
            </div>

            <dl className="gc-evidence-lens__ledger-facts">
              <div><dt>참고치</dt><dd>{record.reference}</dd></div>
              <div><dt>원문 위치</dt><dd>{record.sourceLocation}</dd></div>
              <div><dt>확인 결과</dt><dd>{corrected ? "사용자가 값을 수정함" : "사용자가 원문과 같다고 확인함"}</dd></div>
            </dl>

            <footer>
              <span>출처 · {record.sourceName}</span>
              <span>이 값만으로 건강 상태를 판단할 수 없어요</span>
            </footer>
          </article>

          <aside className="gc-evidence-lens__source" aria-labelledby="source-preview-title">
            <div className="gc-evidence-lens__source-heading">
              <span><DocumentIcon /></span>
              <div>
                <p>원문 위치</p>
                <h2 id="source-preview-title">예시 결과지의 표시 위치예요</h2>
              </div>
            </div>

            <div className="gc-evidence-lens__paper" aria-label="예시 건강검진 결과지 미리보기">
              <header>
                <span>2026 건강검진 결과 통보서</span>
                <span>예시 · 02</span>
              </header>
              <div className="gc-evidence-lens__paper-grid" aria-hidden="true">
                <span>검사항목</span><span>결과</span><span>참고치</span>
                <span>공복혈당</span><span>96</span><span>70–99</span>
                <strong>{record.label}</strong><strong>{record.originalValue}</strong><strong>{record.reference.replace(` ${record.unit}`, "")}</strong>
                <span>비타민 D</span><span>31</span><span>30–100</span>
              </div>
              <p>{record.sourceLocation} · 실제 문서가 아닌 예시 화면이에요</p>
            </div>

            <dl className="gc-evidence-lens__source-meta">
              <div><dt>기관·문서</dt><dd>{record.sourceName}</dd></div>
              <div><dt>검사일</dt><dd><time dateTime={record.observedAt}>{formatKoreanDate(record.observedAt)}</time></dd></div>
              <div><dt>파일 확인값</dt><dd>{record.sourceDigest}</dd></div>
            </dl>
          </aside>
        </section>

        <section className="gc-evidence-lens__history" aria-labelledby="history-title">
          <div>
            <p>확인 과정 4단계</p>
            <h2 id="history-title">값을 확인하는 과정</h2>
          </div>
          <ol>
            <li>
              <span>01</span>
              <div><strong>원문 위치 기록</strong><p>{record.sourceLocation}과 파일 확인값을 함께 표시해요.</p></div>
              <time dateTime={record.observedAt}>{formatKoreanDate(record.observedAt)}</time>
            </li>
            <li>
              <span>02</span>
              <div>
                <strong>자동 추출 예시</strong>
                <p>{record.originalValue} {record.unit}을 예시 후보로 보여드려요. 선택한 파일에서 읽은 값은 아니에요.</p>
                {record.automation && (
                  <details className="gc-evidence-lens__automation">
                    <summary>검토 중인 자동 처리 계획 보기</summary>
                    <p>아래 도구는 아직 이 예시 기록을 만드는 데 사용하지 않았어요.</p>
                    <dl>
                      <div><dt>검토 중인 문서 인식 모델</dt><dd>{record.automation.layoutModel}</dd></div>
                      <div><dt>검토 중인 의료 언어 모델</dt><dd>{record.automation.semanticModel}</dd></div>
                      <div><dt>출시 전 평가 기준</dt><dd>{record.automation.evaluationGate}</dd></div>
                      <div><dt>실행 방식</dt><dd>{record.automation.executionBoundary}</dd></div>
                      <div><dt>실행 파일 확인</dt><dd>{record.automation.artifactPolicy}</dd></div>
                      <div><dt>결과 처리 원칙</dt><dd>{record.automation.disposition}</dd></div>
                    </dl>
                  </details>
                )}
              </div>
              <span>예시 단계</span>
            </li>
            <li data-emphasis="true">
              <span>03</span>
              <div>
                <strong>{corrected ? "사용자가 값을 수정함" : "사용자가 원문과 같다고 확인함"}</strong>
                <p>{corrected ? `${record.originalValue}에서 ${record.value} ${record.unit}로 수정하는 예시예요.` : `${record.value} ${record.unit}이 원문과 같다고 확인하는 예시예요.`}</p>
              </div>
              <time dateTime={record.confirmedAt}>{formatKoreanDateTime(record.confirmedAt)}</time>
            </li>
            <li>
              <span>04</span>
              <div><strong>저장 형태 미리보기</strong><p>값, 단위, 검사일, 원문 위치, 확인 이력을 함께 보여줘요.</p></div>
              <span>예시</span>
            </li>
          </ol>
        </section>

        <aside className="gc-evidence-lens__boundary">
          <span aria-hidden="true">!</span>
          <div>
            <h2>이 화면은 진단 결과가 아니에요</h2>
            <p>참고치는 검사실과 검사 조건에 따라 달라질 수 있어요. 앎은 출처와 확인 과정을 보여주지만 정상·비정상이나 질환을 판정하지 않아요.</p>
          </div>
        </aside>

        <footer className="gc-evidence-lens__actions">
          <button type="button" className="gc-button gc-button--primary" onClick={onBack}>홈으로 돌아가기</button>
        </footer>
      </div>
    </main>
  );
}
